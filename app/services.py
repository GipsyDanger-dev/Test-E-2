...
class RuleBasedMockAnalyzer:
    ...
    def analyze(self,email,attachment_text):
        ...
        if 'sync job failed' in lower or 'oauth token expired' in lower:
            f.project_type='HubSpot sync job'; f.requested_timeline='failed at 02:14'; f.team_size=146
        ...
        if 'correcting my number' in lower:
            p=phones(email.body); f.contact_name='Sam'; f.company_name='Harbour Coldstores'; f.corrected_phone=p[0] if p else None; f.previous_phone=p[1] if len(p)>1 else None; f.preferred_email=sender_email(email.from_raw)
        ...
        if 'invoice' in lower and ('purchase order' in lower or re.search(r'\bpo\b',lower)):
            f.company_name='Greenfields Foods Pty Ltd'; f.contact_name=sender_name(email.from_raw); f.invoice_number=(re.search(r'invoice\s+(\d+)',text,re.I).group(1) if re.search(r'invoice\s+(\d+)',text,re.I) else None); po=re.search(r'purchase order:\s*([^\n]+)',attachment_text or '',re.I); f.purchase_order=po.group(1).strip() if po else None; f.po_value=money(attachment_text or '','approved value'); f.invoice_value=money(attachment_text or '',r'invoice\s*1847'); f.discrepancy_value=f.invoice_value-f.po_value if f.invoice_value and f.po_value else 2640; f.location='Geelong'; f.project_type='lighting project'; f.requested_timeline='before Friday'
        ...
        if 'crew' in lower and ('ballarat' in lower or 'availability' in lower):
            f.company_name='Solara Installations'; f.contact_name=sender_name(email.from_raw); f.team_size=4; f.location='Ballarat'; f.project_type='commercial solar project'; f.requested_timeline='week beginning 14 September; confirmation needed by Tuesday'
        ...
        if 'northbank college' in lower or 'fluorescent fittings' in lower:
            f.company_name='Northbank College'; f.project_type='LED lighting upgrade'; f.service_interest=['LED']; f.team_size=1100
